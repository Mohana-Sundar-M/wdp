...
$sql = "CREATE TABLE IF NOT EXISTS `tbl_product`(
    ...
    PRIMARY KEY(`id`))";
if($connect->query($sql)===TRUE){
    //echo "Table tbl_product created sucesssfully ".'<br>';
}else{
    echo "Error Creating Table ".$connect->error;
}
$sql = "INSERT INTO `tbl_product`(`id`,`name`,`image`,`price`)VALUES
(1,'Samsung J2 Prp','1.jpg',100.00),
...
<html>
    <head>
        <title>MohanaSundar | Shopping Cart</title>
</head>
<body align="center">
    <br />
    ...
    if(mysqli_num_rows($result)>0){
        while($row = mysqli_fetch_array($result)){ ?>
            <div class="col-md-4">
                <form method="post" action ="shop.php?action=add&id=<?php
                echo $row["id"];?>">
                <div style="border:1px solid #333;background-color:#f1f1f1;
                border-radius:5px;padding:10px;"align="center">
                <h4 class="text-info"><?php echo $row["name"];?></h4>
                <h4 class="text-danger">$ <?php echo $row["price"];?></h4>
                <input type="text" name="quantity" class ="form-control" vlaue="1"/>
                <input type="hidden" name="hidden_name" value="<?php echo $row["name"];?>"/>
                <input type="hidden" name="hidden_price" value="<?php echo $row["price"];?>"/>
                <input type="submit" name="add_to_cart" style="margin-top:5px;" class="btnbtn-success" value="Add To Cart"/>
        </div>
        </form>
        </div>
    ...
    if(!empty($_SESSION["shopping_cart"])){
        ...
        foreach($_SESSION["shopping_cart"] as $keys=>$values){
            ...
            <tr>
                ...
                <td> <a href="shop.php?action=delete&id=<?php echo 
